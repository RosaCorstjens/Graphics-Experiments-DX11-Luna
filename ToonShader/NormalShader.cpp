...
const D3D11_INPUT_ELEMENT_DESC NormalShader::_inputLayoutDesc[2] =
{
...
NormalShader::NormalShader(const NormalShader&) {}
NormalShader::~NormalShader() {}

bool NormalShader::Initialize(ID3D11Device* device, HWND hwnd) {
	bool result = InitializeShader(device, hwnd, L"FX/NormalShader.fx");
...
void NormalShader::Shutdown() {
	ReleaseCOM(_FX);
	ReleaseCOM(_inputLayout);
	return;
}

bool NormalShader::Render(ID3D11DeviceContext* deviceContext, int indexCount) {
...
bool NormalShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* filename) {
	...
	_technique = _FX->GetTechniqueByName("Normal");
