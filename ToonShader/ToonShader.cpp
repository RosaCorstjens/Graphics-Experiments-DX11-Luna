...
const D3D11_INPUT_ELEMENT_DESC ToonShader::_inputLayoutDesc[3] =
{
...
ToonShader::ToonShader()
	...
	_worldViewProj(0), _mat(0), _diffTexture(0), _edgeTexture(0) {}

ToonShader::ToonShader(const ToonShader&) {}
ToonShader::~ToonShader() {}

bool ToonShader::Initialize(ID3D11Device* device, HWND hwnd) {
	bool result = InitializeShader(device, hwnd, L"FX/ToonShader.fx");
...
bool ToonShader::Render(ID3D11DeviceContext* deviceContext, int indexCount) {
...
bool ToonShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* filename) {
	...
	_technique = _FX->GetTechniqueByName("Toon");
	...
	_edgeTexture = _FX->GetVariableByName("gEdgeTexture")->AsShaderResource();
