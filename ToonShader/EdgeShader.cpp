...
const D3D11_INPUT_ELEMENT_DESC EdgeShader::_inputLayoutDesc[3] =
{
...
: _FX(0), _technique(0), _inputLayout(0), _world(0), _worldInvTranspose(0), 
_worldViewProj(0), _eyePosW(0), 
_diffTexture(0), _normalTexture(0), _depthTexture(0) {}

EdgeShader::EdgeShader(const EdgeShader&) {}
EdgeShader::~EdgeShader() {}

bool EdgeShader::Initialize(ID3D11Device* device, HWND hwnd) {
	bool result = InitializeShader(device, hwnd, L"FX/EdgeShader.fx");
...
void EdgeShader::Shutdown() {
	ReleaseCOM(_FX);
	ReleaseCOM(_inputLayout);
	return;
}

bool EdgeShader::Render(ID3D11DeviceContext* deviceContext, int indexCount) {
...
bool EdgeShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* filename) {
	...
	_technique = _FX->GetTechniqueByName("EdgeEnhancementStalker");
	...
	_diffTexture = _FX->GetVariableByName("gDiffuseTexture")->AsShaderResource();
	_normalTexture = _FX->GetVariableByName("gNormalTexture")->AsShaderResource();
	_depthTexture = _FX->GetVariableByName("gDepthTexture")->AsShaderResource();

	_eyePosW = _FX->GetVariableByName("gEyePosW")->AsVector();
	_screenWidth = _FX->GetVariableByName("gScreenWidth")->AsScalar();
	_screenHeigth = _FX->GetVariableByName("gScreenHeigth")->AsScalar();
