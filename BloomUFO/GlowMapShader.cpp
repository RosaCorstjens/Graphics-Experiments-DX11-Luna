...
bool GlowMapShader::Initialize(ID3D11Device* device, HWND hwnd) {
	bool result = InitializeShader(device, hwnd, L"FX/GlowMapShader.fx");
...
bool GlowMapShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* filename) {
	...
	_technique = _FX->GetTechniqueByName("GlowMap");
