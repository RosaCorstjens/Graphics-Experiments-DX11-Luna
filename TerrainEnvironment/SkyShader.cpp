...
const D3D11_INPUT_ELEMENT_DESC SkyShader::_inputLayoutDesc[1] =
{
	{ "POSITION", 0, DXGI_FORMAT_R32G32B32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA, 0 }
};

SkyShader::SkyShader() : _cubeMap(0) {}
SkyShader::~SkyShader() {}

bool SkyShader::Initialize(ID3D11Device* device, HWND hwnd) {
	bool result = InitializeShader(device, hwnd, L"FX/Sky.fx");
...
bool SkyShader::Render(ID3D11DeviceContext* deviceContext, int indexCount) {
...
bool SkyShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* filename) {
	...
	_technique = _FX->GetTechniqueByName("Sky");
	...
	_cubeMap = _FX->GetVariableByName("gCubeMap")->AsShaderResource();
